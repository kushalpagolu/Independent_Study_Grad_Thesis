...
from main_thread import run_visualizations_on_main_thread  # Import refactored visualization function
...
from stream_data_fix import EmotivStreamer
...
if __name__ == "__main__":
    ...
    if emotiv.connect():
        ...
        try:
            ...
            save_thread = threading.Thread(target=save_data_continuously, args=(data_store, stop_saving_thread, data_lock, RAW_PATH, PROCESSED_PATH))
            save_thread.daemon = True
            save_thread.start()

            # Start streaming and preprocessing threads
            stream_thread = threading.Thread(target=streaming_thread, args=(emotiv, data_queue, visualization_queue, data_store, data_lock))
            preprocess_thread = threading.Thread(
                target=preprocessing_thread,
                args=(data_queue, feature_queue, env, model_agent, emotiv, stop_main_loop, feature_data_store, lstm_handler, MODEL_FILENAME, session_training_data, data_store, data_lock, RAW_PATH, PROCESSED_PATH))
